use crate::{graphics, terrain};
use nalgebra_glm::{make_vec3, triangle_normal};
use std::convert::TryInto;
use xp_ui::{Widget, UI};

pub fn create_terrain_mesh_from_tile(
    tile: &terrain::Tile,
) -> graphics::Mesh<graphics::default::Vertex> {
    let mut terrain = graphics::Mesh {
        vertices: Vec::new(),
        indices: Vec::new(),
    };
        ...
        for z in 0..S {
            ...
            match (x, z) {
                (_, 0) => {
                    ...
                    terrain.vertices[index as usize].normal = triangle_normal(
                        &make_vec3(&terrain.vertices[p1 as usize].position),
                        &make_vec3(&terrain.vertices[p2 as usize].position),
                        &make_vec3(&terrain.vertices[p3 as usize].position),
                    )
                    .as_slice()
                    .try_into()
                    .unwrap();
                ...
                (_, _) => {
                    ...
                    terrain.vertices[p4 as usize].normal = triangle_normal(
                        &make_vec3(&terrain.vertices[p4 as usize].position),
                        &make_vec3(&terrain.vertices[p5 as usize].position),
                        &make_vec3(&terrain.vertices[p6 as usize].position),
                    )
                    .as_slice()
                    .try_into()
                    .unwrap();
