...
void main() {
    ...
    vec2 position = vec2(in_position.x + camera_position_floor.x - 128, in_position.y + camera_position_floor.z - 128);
    out_color = vec3(1.0, 1.0, 0.0);
    vec2 uv = vec2(position.x / 256.0, position.y / 256.0); // tile_size = clipmap_size - 1, meaning 1 value is not used
